...
dotenv.load_dotenv()
os.environ["GPT_MODEL"] = os.environ["GPT_MODEL"].replace("gpt-3.5-turbo", "gpt-3.5-turbo-1106", 1).replace("gpt-4-turbo-vision", "gpt-4-vision-preview", 1).replace("gpt-4-turbo", "gpt-4-1106-preview", 1)
...
MAX_TOTAL_TOKENS = {"gpt-3.5-turbo-1106": 16385, "gpt-4-1106-preview": 128000, "gpt-4-vision-preview": 128000}
...
def count_tokens(msg, num_tokens=EXTRA_TOKENS_PER["message"]):
    for key, value in msg.items():
        if key == "content" and isinstance(value, list):
            for item in value: num_tokens += count_tokens(item, num_tokens=0)
        elif key not in ("image_url", "type"): num_tokens += len(encoding.encode(value))
        if key in EXTRA_TOKENS_PER: num_tokens += EXTRA_TOKENS_PER[key]
    return num_tokens
...
async def on_message(message):
    ...
    async with message.channel.typing():
        ...
        current_msg = message
        while True:
            current_msg_text = current_msg.embeds[0].description if current_msg.author == discord_client.user else current_msg.content
            if current_msg_text.startswith(discord_client.user.mention): current_msg_text = current_msg_text[len(discord_client.user.mention):].lstrip()
            current_msg_content = [{"type": "text", "text": current_msg_text}] if current_msg_text else []
            if os.environ["GPT_MODEL"] == "gpt-4-vision-preview": current_msg_content += [{"type": "image_url", "image_url": {"url": att.url, "detail": "low"}} for att in current_msg.attachments if "image" in att.content_type]
            ...
            try: current_msg = current_msg.reference.resolved if isinstance(current_msg.reference.resolved, discord.Message) else await message.channel.fetch_message(current_msg.reference.message_id)
        ...
        logging.info(f"Generating response for content:\n{msgs[-1]['content']}")
        ...
        async for part in await openai_client.chat.completions.create(model=os.environ["GPT_MODEL"], messages=msgs, max_tokens=MAX_COMPLETION_TOKENS, stream=True):
            ...
            if "previous_content" in locals() and previous_content:
                if response_messages == [] or len(response_message_contents[-1]+previous_content) > EMBED_MAX_LENGTH:
                    ...
                    response_messages += [await reply_message.reply(embed=discord.Embed(description=previous_content, color=embed_color), silent=True)]
